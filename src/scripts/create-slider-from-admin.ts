...
async function createSliderFromLocal() {
  try {
    console.log("🚀 Creating slider entries using local images as fallback...");
    ...
    for (const imageData of sliderImages) {
      ...
      const imageUrl = `/ch/${imageData.filename}`;
    ...
    console.log("\n📋 Note: Using local images from /ch folder");
    console.log(
      "💡 To use Supabase Storage later, make the bucket public and run 'npm run create:slider'"
    );
  } catch (error) {
