                ...
                <div className="relative flex items-center">
                  ...
                  <div className={`w-1/2 ${index % 2 === 0 ? 'pr-8 text-right' : ''}`}>
                    {index % 2 === 0 && (
                      <Link href={`/achievements/${item.id}`} className="block">
                        <div className="relative ml-auto w-full max-w-md rounded-xl border bg-card p-6 shadow-lg transition-transform duration-300 hover:scale-105 hover:shadow-xl">
                  ...
                  {/* Right Side */}
                   <div className={`w-1/2 ${index % 2 !== 0 ? 'pl-8 text-left' : ''}`}>
                    {index % 2 !== 0 && (
                       <Link href={`/achievements/${item.id}`} className="block">
                        <div className="relative mr-auto w-full max-w-md rounded-xl border bg-card p-6 shadow-lg transition-transform duration-300 hover:scale-105 hover:shadow-xl">
                          <p className="text-sm font-semibold text-primary">{item.year}</p>
                          <h3 className="mt-1 text-xl font-bold text-foreground">{item.title}</h3>
                          <p className="mt-2 text-foreground/70">{item.description}</p>
                        </div>
